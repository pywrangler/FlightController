    ...
    if evbuf:
        ...
        if(type == 2):  #type 2 means stick movement or throt wheel
            ...
            if(num==1 ):
                if(val == 1):
                    rfsp = fsp
                    fsp = 160 #servowrite(160) means full throttle
                else:
                    fsp = rfsp
        ...
        s1 =max(min(((((max(min(32760, (base-diff) ), -32760))/32760.0)+1)*Lutrw)+Ltr,180),0)
        s2 = max(min(((((max(min(32760, (diff+base) ), -32760))/32760.0)+1)*Rutrw)+Rtr,180),0)
        #fsp = ((32768-fsp) * 110 / 65596)+56
        #(x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
        # s1 = (max(min(32760, (base+diff) ), -32760))
        # s2 = (max(min(32760, (base-diff) ), -32760))
        # print "plus ",base+diff, " minus ", base-diff
        # print " s1 is ",s1," s2 is ",s2
        cd = ('%03d' % s1)+('%03d' % s2)+('%03d' % fsp)
        #print " time: ",time," val ",val," type ",type," num ",num
        print cd
        # sock.sendto( cd, (espIP, espPort))
        # print "the pressed is",number," val ",value
        # if type & 0x80:
        #      print "(initial)",
        #
        # if type & 0x01:
        ...
        #         sock.sendto( str((85+(fvalue*85))), (UDP_IP, UDP_PORT))
        #         #print "%s: %.3f" % (axis, (85+(fvalue*85)) )
